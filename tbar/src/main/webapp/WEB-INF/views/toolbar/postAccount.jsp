...
<div class="mask_layout" style="display: none;">
    <div class="setting_close">
        ...
        <div class="btn_inline_block">
            <button class="btn_ov btn_mid cancel" style="font-size: .34rem;" id="cancel">Cancel</button>  <button class="btn_ov btn_mid ok" id="apply" style="font-size: .34rem;">Apply</button>
        </div>
...
<script type="text/javascript">
	...
	var browseUrl = "toolbar/postAccount.jsp";
    ...
    window.onload=function () {
        ...
        $(".setting_ul .setting_item").on("click",function(){
            recordClickEvent('${suid}','toolbar/postAccount.jsp/setting_item');
...
//             $(this).hasClass("active")?$(this).removeClass("active"):$(this).addClass("active");
        ...
        recordClickEvent('${suid}','toolbar/postAccount.jsp/apply');
        	$.ajax({
				type: "post",
				async: false,
        ...
        recordClickEvent('${suid}','toolbar/postAccount.jsp/closeService');
            $(".mask_layout").slideDown();
        });

        $(".mask_layout").on("click",function(){
        recordClickEvent('${suid}','toolbar/postAccount.jsp/mask_layout');
            $(this).slideUp();
        });
        $("#cancel").on("click",function(){
        recordClickEvent('${suid}','toolbar/postAccount.jsp/cancel');
            $(this).parents(".mask_layout").slideUp();
        });

        $(".setting_close").on("click",function(e){
        recordClickEvent('${suid}','toolbar/postAccount.jsp/setting_close');
            e.stopPropagation();
        });
        $("#feedback").on("click",function(){
        recordClickEvent('${suid}','toolbar/postAccount.jsp/feedback');
			window.location.href="<%= basePath%>page/turnFeedback?suid=${suid}";        
        });
    };

	function jumpNotice(){
	recordClickEvent('${suid}','toolbar/postAccount.jsp/jumpNotice');
