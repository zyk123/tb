...
<!--                         <i class="left_current" style="width:<c:choose> <c:when test="${empty remainedData || empty totalData  || remainedData eq 0.00  || totalData eq 0.00 }">0%</c:when> <c:otherwise><fmt:formatNumber value="${(remainedData/totalData)*100 }" type="number" pattern="#"/>%</c:otherwise> </c:choose>"></i> -->
<!--                     </P> -->
<!--                 </div> -->
...
<script type="text/javascript">
    ...
    function invoke(url,id){
        recordClickEvent('${param.suid}','toolbar/preStore.jsp/'+id);
...
// 		window.open("<%= basePath%>page/turnPackageDetailOrder?" + $('#store_form').serialize());
