...
<script>
	$(function(){
		...
		$('#add_btn').on('click',function(){
			if($('#add_form').validationEngine('validate') && flag){
				flag = false;
				$.ajax({
				url:'${ctx}/blackListDevice/addDevice',
				type:'post',
				data:{
					name:$.trim($('#name').val()),
					type:$.trim($('#type').val())
	...
	$('#del_btn').on('click',function(){
		if($('.table_content input:checked').length == 0){
       	...
       	if(confirm("Are you sure you want to delete the selected data?")){
   			...
   			$.ajax({
   				url:ctx + '/blackListDevice/delDevice',
   				type:'post',
   				data:{array:array},
   				traditional:true,
   				dataType:'json',
	...
	$('.d_mod').on('click',function(){
		if('Modify' == $(this).text()){
			...
			if(val == value||$('#t_form').validationEngine('validate')){				
				$.ajax({
				url:'${ctx}/blackListDevice/updateDevice',
				type:'post',
				data:{
					id:obj.prev().val(),
					name:$.trim(val)
	...
	function nextPage(currentPage){
		window.location.href="${ctx}/blackListDevice/showDeviceList?" + $('#search_form').serialize()
