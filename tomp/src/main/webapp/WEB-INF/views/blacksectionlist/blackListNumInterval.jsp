			...
			$.ajax({
				...
				success: function(data) {				
					...
					$.each(result,function(i,item){
						...
						html += '<td><input type="checkbox" onclick="changeCheckAll(this)"><input type="hidden" value="'+item.bean.bSectionId+'">';			
						html += '<span>'+item.bean.mobileNoStart+'</span></td>';
						html += '<td>'+item.bean.mobileNoEnd+'</td>';			
		...
		function deleteBlackList(){
			...
			if(window.confirm("Are you sure you want to delete the selected data?")){
				...
				$.ajax({
					type: "POST",
					cache:false,
					url: "${ctx}/blackSegmentList/delete",
					data: {
						"delIds":delIds
						},
					dataType: "json",
...
<input type="text" id="htmlIdentifier" value="add" style="display: none;" />
 <c:set var="t"  value=""/>
    ...
    <div class="table_search">
	        ...
	        <form id="conditionForm">
	            <li>phone number <input class="phone" id="mobileNo" name="mobileNo" type="text" data-validation-engine=="validate[custom[posInteger],,maxSize[12]]"></li>
	            ...
	            <li>batch No <input class="num" name="batchNo" id="batchNo" type="text" data-validation-engine=="validate[custom[lengthNumber]]"></li>
	            <input type="hidden" name="pageIndex" id="pageIndex" value="1"/>
	            <input type="hidden" name="pageRowNum" id="pageRowNum" value="10"/>	  	            
	        </form>
	        	<c:if test="${!empty blackSegmentListPrivileges && fn:contains(blackSegmentListPrivileges,'SEARCH') }">
	            <li><button class="btn_ov mid_btn search" onclick="blackList()">query</button></li>
	            </c:if>
	            <c:if test="${!empty blackSegmentListPrivileges && fn:contains(blackSegmentListPrivileges,'DELETE') }">
            	<li><button class="btn_ov mid2_btn delete" onclick="deleteBlackList()">delete</button></li>
            	</c:if>
            	<c:if test="${!empty blackSegmentListPrivileges && fn:contains(blackSegmentListPrivileges,'IMPORT') }">
            		<li><form id="upload_form"  target="upload_frame" action="<%=path %>/blackSegmentList/importBlackSectionList" enctype="multipart/form-data" method="post"><div class="btn_ov mid2_btn import"  style="position: relative;">import<input type="file" id="fileImport" name="fileImport" style="opacity:0; width: 100%; height: 100%; cursor: pointer; top:0; left: 0; position: absolute;" value="选择文件" /></div></form></li>
 ...
 <table class="table bor" id="listTable">
     <tr class="table_title">
         <td><input class="checkall" id="checkall" type="checkbox"  style="left:30px;position: absolute;">start number</td>
         <td>end number</td>
         <td>creation time</td>
         <td>batch No</td>
     </tr>
